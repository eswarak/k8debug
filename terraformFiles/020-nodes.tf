...
resource "ibm_compute_vm_instance" "compute" {
  ...
  datacenter           = "${var.datacenter}"
  disks                = ["${var.compute["disk_size"]}","${var.gluster_disk_size}"]
  domain               = "${var.cloud_domain}"
...
resource "ibm_compute_vm_instance" "student" {
  lifecycle {
    ignore_changes     = ["private_vlan_id"]
  }

  cores                = "${var.student["cpu_cores"]}"
  count                = "${var.student["nodes"]}"
  datacenter           = "${var.datacenter}"
  disks                = ["${var.student["disk_size"]}"]
  domain               = "${var.cloud_domain}"
  hostname             = "${format("%s-%s-%01d", lower(var.host_base), lower(var.student["name"]),count.index + 1 )}"
  hourly_billing       = "${var.student["hourly_billing"]}"
  local_disk           = "${var.student["local_disk"]}"
  memory               = "${var.student["memory"]}"
  network_speed        = "${var.student["network_speed"]}"
  os_reference_code    = "${var.os_reference}"
  private_network_only = "${var.student["private_network_only"]}"
  #private_vlan_id      = "${ibm_compute_vm_instance.master.0.private_vlan_id}"     // notice this uses the master private_vlan_id
  ssh_key_ids          = ["${ibm_compute_ssh_key.ibm_public_key.id}"]
  tags                 = ["${var.student_tag}"]
