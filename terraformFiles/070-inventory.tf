...
data "template_file" "inventory" {
  ...
  vars {
    ...
    node4_ip         = "${ibm_compute_vm_instance.compute.3.ipv4_address}"
    node5_ip         = "${ibm_compute_vm_instance.compute.4.ipv4_address}"
    node6_ip         = "${ibm_compute_vm_instance.compute.5.ipv4_address}"
    cluster_id       = "${var.cluster_id}"
    version          = "${var.okd_version}"
    gfs_node1_ip     = "${ibm_compute_vm_instance.compute.0.ipv4_address}"
    gfs_node2_ip     = "${ibm_compute_vm_instance.compute.1.ipv4_address}"
    gfs_node3_ip     = "${ibm_compute_vm_instance.compute.2.ipv4_address}"
    gfs_device       = "${var.gluster_device}"
  }
}
