...
output "student_public_ip" {
  value = ["${ibm_compute_vm_instance.student.*.ipv4_address}"]
}
output "student_private_ip" {
  value = ["${ibm_compute_vm_instance.student.*.ipv4_address_private}"]
}
...
resource "null_resource" "end" {
  depends_on = ["null_resource.okd_build", "null_resource.student_oc"]
